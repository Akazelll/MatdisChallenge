...
class VisualisasiGraf:
    """
    Sebuah kelas untuk membuat dan memvisualisasikan graf berdasarkan input pengguna,
    mengikuti logika dari flowchart yang diberikan. Mampu menangani sisi ganda dan loop.
    """
    def __init__(self):
        """Inisialisasi properti graf."""
        self.simpul = []
        self.derajat = {}
        # Menggunakan MultiGraph adalah kunci untuk bisa menyimpan sisi ganda dan loop
        self.graf = nx.MultiGraph()

    def _dapatkan_input_integer(self, prompt, min_val=0):
        """Metode bantuan internal untuk mendapatkan input integer yang valid."""
        while True:
            try:
    ...
    def _visualisasikan(self, judul="Visualisasi Graf"):
        """Metode internal untuk mencetak visualisasi graf."""
        print("\nMencetak visualisasi...")
        pos = nx.spring_layout(self.graf, seed=42)
        plt.figure(figsize=(10, 8))
        
        nx.draw(self.graf, pos, with_labels=True, node_color='skyblue', node_size=2000, 
                edge_color='gray', width=1.5, font_size=12, font_weight='bold')

        try:
            labels = {node: f'd={degree}' for node, degree in self.graf.degree()}
            pos_labels = {key: (value[0], value[1] - 0.1) for key, value in pos.items()}
            nx.draw_networkx_labels(self.graf, pos_labels, labels=labels, font_size=10, font_color='red')
    ...
    def _konstruksi_havel_hakimi(self, urutan_derajat):
        """Implementasi algoritma Havel-Hakimi untuk graf sederhana."""
    ...
    def _jalur_tanpa_syarat(self):
        """
        Logika ini secara alami dapat menghasilkan SISI GANDA dan LOOP.
        """
        ...
        while len(stubs) > 1:
            u = stubs.pop()
            v = stubs.pop()
            # Jika pasangan (u, v) ini muncul lebih dari sekali,
            # maka SISI GANDA akan terbentuk karena kita menggunakan MultiGraph.
    ...
    def _jalur_dengan_syarat(self):
        """
        Logika ini secara eksplisit meminta input untuk membuat SISI GANDA.
        """
        print("\n--- Opsi 2: Membuat Graf Dengan Syarat ---")
        derajat_sisa = self.derajat.copy()

        # Input SISI GANDA secara eksplisit
            ...
            while True:
                ...
                try:
                    ...
                    if u in self.simpul and v in self.simpul and u != v:
                        # Setiap kali add_edge dipanggil untuk pasangan yang sama,
                        # sisi baru akan ditambahkan (SISI GANDA).
                        self.graf.add_edge(u, v)
                        derajat_sisa[u] -= 1
                        derajat_sisa[v] -= 1
