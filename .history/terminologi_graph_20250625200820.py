...
class VisualisasiGraf:
    ...
    # __init__, _dapatkan_input_integer, _visualisasikan, dan _jalur_tanpa_syarat
    ...
    # --- FUNGSI LAMA (manual) BISA DIHAPUS ATAU DISIMPAN JIKA PERLU ---
    # def _jalur_dengan_syarat_manual(self):
    # ...
    
    # === FUNGSI BARU YANG DI-IMPROVE (OTOMATIS) ===
    def _jalur_dengan_syarat_otomatis(self):
        ...
        for i in range(jml_loop):
            ...
            if not kandidat_loop:
                print(f"Peringatan: Tidak dapat membuat loop ke-{i+1} karena tidak ada simpul dengan sisa derajat >= 2.")
                break
            
            # Pilih simpul secara acak dari kandidat
        ...
        # 3. Membuat sisi ganda secara acak
        for i in range(jml_sisi_ganda):
            # Cari simpul yang bisa dibuatkan sisi (sisa derajat >= 1)
            kandidat_sisi = [node for node, deg in derajat_sisa.items() if deg >= 1]
            if len(kandidat_sisi) < 2:
                print(f"Peringatan: Tidak bisa membuat sisi ganda ke-{i+1} karena kurang dari 2 simpul dengan sisa derajat.")
                break

            # Pilih 2 simpul berbeda secara acak
        ...
        # 4. Melengkapi sisa graf dengan Model Konfigurasi
    ...
    def jalankan(self):
        ...
        print("2. Cetak dengan syarat (menentukan jumlah loop & sisi ganda secara otomatis)") # Deskripsi diubah
        ...
        if choice == '1':
            ...
            # Panggil fungsi baru yang sudah otomatis
