...
def validate_degrees(degrees, num_nodes):
    """
    Validasi derajat simpul.
    - Jumlah total derajat harus genap.
    - Derajat simpul tidak boleh lebih besar dari jumlah simpul lain.
    """
    if sum(degrees) % 2 != 0:
        print("Jumlah total derajat harus genap. Silakan periksa kembali input Anda.")
        return False
    if any(degree >= num_nodes for degree in degrees):
        print("Derajat simpul tidak boleh lebih besar dari jumlah simpul lain. Silakan periksa kembali input Anda.")
        return False
    return True
...
def create_graph():
    ...
    while True:
        ...
        if validate_degrees(degrees, num_nodes):
            break
