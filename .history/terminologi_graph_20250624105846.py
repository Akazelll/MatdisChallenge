...
def buat_graf_otomatis():
    ...
    # Membuat sisi untuk memenuhi derajat
    sisi_tersisa = Counter(derajat)
    while any(sisi_tersisa.values()):
        simpul_aktif = [node for node, sisa in sisi_tersisa.items() if sisa > 0]
        if len(simpul_aktif) > 1:
            u, v = np.random.choice(simpul_aktif, size=2, replace=False)
        elif len(simpul_aktif) == 1:  # Hanya satu simpul dengan derajat tersisa
            u = v = simpul_aktif[0]
        else:
            break

        # Tambahkan sisi atau loop
        G.add_edge(u, v)
        sisi_tersisa[u] -= 1
        sisi_tersisa[v] -= 1

    # Memeriksa apakah semua derajat terpenuhi
    if any(sisi_tersisa.values()):
        print("Tidak dapat memenuhi semua derajat sesuai input. Program selesai.")
        return G
